...
class MISA_T_A(nn.Module):
    
    def __init__(self, args):
        print("Backbone: T-A---000000")
        #exit()

        super(MISA_T_A, self).__init__()
        ...
        self.fusion.add_module('fusion_layer_1', nn.Linear(in_features=args.hidden_size * 4, out_features=args.hidden_size * 2))
        self.fusion.add_module('fusion_layer_1_dropout', nn.Dropout(self.dropout_rate))
        self.fusion.add_module('fusion_layer_1_activation', self.activation)
        self.fusion.add_module('fusion_layer_3', nn.Linear(in_features=args.hidden_size * 2, out_features= self.output_dim))

        self.gate = nn.Sequential() #fusion---
        self.gate.add_module('fusion_layer_1', nn.Linear(in_features=args.hidden_size * 4, out_features=args.hidden_size * 2))
        self.gate.add_module('private_t_activation_1', nn.Sigmoid())
        #self.fusion.add_module('fusion_layer_1_dropout', nn.Dropout(self.dropout_rate))
        #self.fusion.add_module('fusion_layer_1_activation', self.activation)
        #self.fusion.add_module('fusion_layer_3', nn.Linear(in_features=args.hidden_size * 2, out_features= self.output_dim))
    ...
    def forward(self, text_feats, video_feats, audio_feats):
        ...
        #print(bert_sent)
        #print(bert_sent.shape)
        #print(bert_sent_mask)
        #print(bert_sent.shape)
        #print(bert_sent_type)
        #print(bert_sent_type.shape)
        #exit()
        bert_output = self.text_subnet(text_feats) # [batch_size, seq_len, 768]
        #alal=bert_sent_mask.unsqueeze(2)
        #print(alal.shape)
        #print(bert_output.shape)
        #exit()

        # Use the mean value of bert of the front real sentence length as the final representation of text.
        masked_output = torch.mul(bert_sent_mask.unsqueeze(2), bert_output)  #---wise Multiplication
        ...
        #exit()
        text_feats = torch.sum(masked_output, dim=1, keepdim=False) / mask_len  #sum
        #print(text_feats.shape)
        #exit()
        ...
        #print(final_h1a.shape)
        #print(final_h2a.shape)
       ...
       # print(audio_feats.shape)
        ...
        self.shared_or_private_s = self.sp_discriminator((self.utt_shared_t  + self.utt_shared_a) / 2.0)

        self._reconstruct()

        h = torch.stack((self.utt_private_t, self.utt_private_a, self.utt_shared_t,  self.utt_shared_a), dim=0)
        h = self.transformer_encoder(h)
        h0 = torch.cat((h[0], h[1], h[2], h[3]), dim=1)
        h1= torch.cat((h[0], h[2]), dim=1)
        h2= torch.cat((h[1], h[3]), dim=1)
        hh=torch.cat((h1,self.gate(h0)*h2),dim=1)
        logits = self.fusion(hh)
